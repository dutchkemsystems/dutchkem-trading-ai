...
class M5ScalpingStrategy(TimeframeStrategy):
    ...
    def calculate(self, ohlcv_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stoch_k, stoch_d = calculate_stochastic(high, low, close, 5, 3, 3)
...
class M30SwingStrategy(TimeframeStrategy):
    ...
    def calculate(self, ohlcv_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        supertrend, direction = calculate_supertrend(high, low, close, 10, 3)
...
class H1TrendStrategy(TimeframeStrategy):
    ...
    def calculate(self, ohlcv_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        ichimoku = calculate_ichimoku(high, low, close, 9, 26, 52)
        ...
        if (
            close[-1] > ichimoku["senkou_a"]
            and close[-1] > ichimoku["senkou_b"]
            and ema_20 > ema_50
            and rsi > 50
        ):
            signal = "BUY"
...
class H2PositionStrategy(TimeframeStrategy):
    ...
    def calculate(self, ohlcv_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        macd_line, signal_line, histogram = calculate_macd(close, 12, 26, 9)
        psar = calculate_psar(high, low, close, 0.02, 0.2)
        adx = calculate_adx(high, low, close, 14)
...
class H4StrategicStrategy(TimeframeStrategy):
    ...
    def calculate(self, ohlcv_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if ema_50 > ema_200 and close[-1] > ichimoku["senkou_a"] and close[-1] > ichimoku["senkou_b"]:
            signal = "BUY"
            strength = min(100, ((ema_50 - ema_200) / ema_200 * 10000) + 50)
        elif ema_50 < ema_200 and close[-1] < ichimoku["senkou_a"] and close[-1] < ichimoku["senkou_b"]:
            signal = "SELL"
